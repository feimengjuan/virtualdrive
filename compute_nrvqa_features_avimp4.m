function all_features = compute_nrvqa_features_avimp4(test_video)

blk_len = 25;
obj = VideoReader(test_video);
nFrames = obj.NumberOfFrames;
nRate = obj.FrameRate;
frame_start = 1+1; 
frame_end = floor((nFrames-5+1)/1);
...
for i = frame_start:2:frame_end
    ...
    if first_frame_loaded
        ...
        this_yuv_frame = rgb2ycbcr(double(read(obj, i))./255);
        next_yuv_frame = rgb2ycbcr(double(read(obj, i+1))./255);
    else
        prev_yuv_frame = rgb2ycbcr(double(read(obj, i-1))./255);
        this_yuv_frame = rgb2ycbcr(double(read(obj, i))./255);
        next_yuv_frame = rgb2ycbcr(double(read(obj, i+1))./255);
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    HC_features = [];
    for i=fr_idx
        frame = rgb2ycbcr(double(read(obj,frame_start+(i-1)*2))./255);
        fprintf('Computing HC features for the frame %d\n',...
                frame_start+(i-1)*2);
        ftrs = compute_HC_features(frame);
